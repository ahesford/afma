C* 19-dec-1999 sanjay; changed the names to drivegmres and initgmres to
C*      fix a  silly problem with Linux. On Linux somehow the
C*      underscores are problematic
C*
...
C*
*
        subroutine drivecgmres(n,nloc,m,lwork,work,
...
*
         if (iwarn.ne.0) then
           write(iwarn,*)
           write(iwarn,*) ' WARNING : For M = ',m,' optimal value '     
...
*
         if ((icntl(5).lt.0).or.(icntl(5).gt.3)) then
           ...
           if (iwarn.ne.0) then
             write(iwarn,*)
             write(iwarn,*) ' WARNING : Undefined orthogonalisation '  
         ...
         if ((icntl(6).ne.0).and.(icntl(6).ne.1)) then
           ...
           if (iwarn.ne.0) then
             write(iwarn,*)
             write(iwarn,*) ' WARNING : Undefined intial guess '
...
*
         if ((m .gt. n).or.(lwork.lt.sizeWrk)) then
           if (m .gt. n) then
             ...
             if (iwarn.ne.0) then
               write(iwarn,*)
               write(iwarn,*) ' WARNING : parameter M bigger than N'  
...
* compute the maximum size of the m according to the memory space
     ...
     &                   +1.0-float(lwork))))/2.0)
             if (newRestart.gt.0) then
               ...
               if (iwarn.ne.0) then
                 write(iwarn,*)
                 write(iwarn,*)' WARNING : workspace too small for M'  
                 write(iwarn,*)'           new value for M ',m
                 write(iwarn,*)
               endif
             else
...
*
* Updated : May 1998
*           r0(1) <-- r0'r0 : pb when used with DGEMV for the dot product
*           cure : w(1) <--  r0'r0
*
*  Arguments
*  =========
...
* Intrinsic function
* ------------------
        intrinsic cabs, sqrt, real, conjg
*
*       Executable statements
*
* setup some pointers on the workspace
...
* Apply previous Givens rotations to the new column of H
       do j=1,jH-1
         temp      =  rotCos(j)*H(j,jH)+conjg(rotSin(j))*H(j+1,jH)
         H(j+1,jH) = -rotSin(j)*H(j,jH)+rotCos(j)*H(j+1,jH)
         H(j,jH)   =  temp
       enddo
       call givens(H(jH,jH),H(jH+1,jH),rotCos(jH),rotSin(jH))
* Apply current rotation to the rhs of the least squares problem
       H(jH+1,m+1) = -rotSin(jH)*H(jH,m+1)
       H(jH,m+1)   =  rotCos(jH)*H(jH,m+1)
*
...
* rotation to the last entry of the collumn
       H(jH,jH)   = rotCos(jH)*H(jH,jH)+conjg(rotSin(jH))*H(jH+1,jH)  
       H(jH+1,jH) = ZERO
*
...
C Hastriter added flush so that iterations get updated so completion time 
C can be estimated for big jobs      
           call flush(6)
...
* Return the backward errors
             ...
             if (iterOut*m+jH.lt.iterMax) then
               info(1) = 0
               if (ihist.ne.0) then
                 write(ihist,*)
...
*
        subroutine initcgmres(icntl,cntl)
*
*  Purpose
*  =======
